...
def flatten_clone_data(clone):
    ...
    flat_record = {
        ...
        # Partition info
        "partition_id": clone.get("partition", {}).get("id")
        if isinstance(clone.get("partition"), dict)
        else None,
        "partition_step": clone.get("partition", {}).get("step")
        if isinstance(clone.get("partition"), dict)
        else None,
        # Tree count
...
def flatten_tree_and_node_data(clone, tree_idx, tree, node_id, node):
    ...
    node_data = {
        ...
        "node_cluster_timepoint_multiplicities": json.dumps(
            node.get("cluster_timepoint_multiplicities", [])
        ),
...
def convert_json_to_csv(json_file, csv_file):
            ...
            for tree_idx, tree in enumerate(trees):
                ...
                if not nodes:
                    ...
                    for node_id, node in nodes.items():
                        flat_record = flatten_tree_and_node_data(
                            clone, tree_idx, tree, node_id, node
                        )
                        flat_record.update(metadata)
                        all_records.append(flat_record)
                        total_nodes += 1
    ...
    if all_records:
        ...
        print(
            f"Successfully converted {len(clones)} clones with {total_nodes} total tree nodes to CSV"
        )
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", help="Output CSV file path (default: input_file.csv)"
    )
