...
|---|---|---|
| `--split-files` legacy multi-file output (`datasets.json` + `clones.*.json` + `tree.*.json`) | [#43](https://github.com/matsengrp/olmsted-cli/issues/43), [PR #48](https://github.com/matsengrp/olmsted-cli/pull/48) (merge commit `78aab14`) | Predated the streaming pipeline (#26). `validate.py`'s unrelated `--split` flag was unaffected — a different feature that happened to share a variable name. The separate `olmsted split` *subcommand* was NOT removed here despite the name collision — see below, it went in a later round. |
| Legacy `-f airr` (the original, Olmsted-flavored `{dataset_id, clones, ...}` container) | [#47](https://github.com/matsengrp/olmsted-cli/issues/47), [PR #52](https://github.com/matsengrp/olmsted-cli/pull/52) (merge commit `8dbaf18`; supersedes the auto-closed PR #49 — see #49 for why) | Research found it never corresponded to an official AIRR Community schema release at any version. `-f airr2` (the real AIRR-C v2 Clone/Tree/Node/Cell schema, AIRR Schema v2.0.0) was renamed to take over as the sole `-f airr` — see [FORMATS.md](./FORMATS.md#airr-input-format). |
| `olmsted split` subcommand (chunked a consolidated Olmsted JSON into smaller consolidated-shaped files by clone count) | [#53](https://github.com/matsengrp/olmsted-cli/issues/53) | Already self-labeled "(legacy)" in `cli.py`'s subcommand table before removal. No tests, no internal callers, and no known consumers — likely superseded by the streaming pipeline (#26) and browser-side IndexedDB storage, both of which address the "large dataset" problem this command worked around by a different route. `validate.py`'s unrelated `--split` flag is unaffected. |
