...
def analyze_consolidated_data(data):
    ...
    if all_clones:
        unique_seqs_counts = [
            c.get("unique_seqs_count", 0)
            for c in all_clones
            if "unique_seqs_count" in c
        ]
        read_counts = [
            c.get("total_read_count", 0) for c in all_clones if "total_read_count" in c
        ]
        mut_freqs = [
            c.get("mean_mut_freq", 0) for c in all_clones if "mean_mut_freq" in c
        ]
        ...
        summary["clone_statistics"] = {
            "unique_sequences": {
                ...
                "mean": sum(unique_seqs_counts) / len(unique_seqs_counts)
                if unique_seqs_counts
                else 0,
    ...
    summary["trees"] = {"total_count": len(trees), "reconstruction_methods": {}}
    ...
    summary["gene_usage"] = {
        "v_genes": {"unique_count": len(v_genes), "genes": sorted(list(v_genes))},
        "j_genes": {"unique_count": len(j_genes), "genes": sorted(list(j_genes))},
        "d_genes": {"unique_count": len(d_genes), "genes": sorted(list(d_genes))},
    }
...
Examples:
  ...
  olmsted summary data.json -o summary.txt
    ...
    parser.add_argument("input_file", help="Olmsted JSON file to analyze")
...
def main():
    ...
    if missing_keys:
        vprint.error(
            f"Error: Not a valid Olmsted JSON format. Missing keys: {missing_keys}"
        )
        sys.exit(1)
