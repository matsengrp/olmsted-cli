...
def split_consolidated_data(data, max_clones_per_file):
    ...
    if missing_keys:
        raise ValueError(
            f"Not a valid Olmsted JSON format. Missing keys: {missing_keys}"
        )
    ...
    for chunk_idx, clone_chunk in enumerate(clone_chunks):
        ...
        for dataset in split_data["datasets"]:
            ...
            dataset_clone_count = sum(
                1
                for clone in clone_chunk
        ...
        for tree in all_trees:
            if tree.get("clone_id") in chunk_clone_ids:
                ...
                if updated_tree.get("dataset_id") in old_to_new_dataset_mapping:
                    updated_tree["dataset_id"] = old_to_new_dataset_mapping[
                        updated_tree["dataset_id"]
                    ]
...
Examples:
  ...
  olmsted split -i data.json -o splits --max-clones 50 --base-name my_dataset
    ...
    parser.add_argument(
        "-i", "--input", required=True, help="Input Olmsted JSON file to split"
    )

    parser.add_argument(
        "-o", "--output-dir", required=True, help="Output directory for split files"
    )
...
def main():
    ...
    if len(split_files) == 1:
        vprint.status(
            f"File has {sum(len(clones) for clones in data['clones'].values())} clones, "
    ...
    for i, split_data in enumerate(split_files):
        ...
        try:
            ...
            if args.verbose:
                clone_count = sum(
                    len(clones) for clones in split_data["clones"].values()
                )
                tree_count = len(split_data["trees"])
                vprint.status(
                    f"Created {output_filename}: {clone_count} clones, {tree_count} trees"
                )
    ...
    vprint.status(
        f"Successfully split {input_path.name} into {len(split_files)} files:"
    )
